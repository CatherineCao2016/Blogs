...
Prefer to watch first? Jump to [See it in action](#see-it-in-action) for a full walkthrough video.
...
You can think of it as an isolated sandbox in the cloud, already set up for the task at hand. Instead of relying on your laptop's CPU, memory, and local environment, you send work there and let it run. Because the sandbox is isolated and its network access is something you configure, you can let the agent work on its own without watching every step.
...
Configuration Sync is how you make that personal configuration available to your cloud sessions.

## When to use a cloud session

With Kiro Web and Mobile you are always working through a cloud session, so the choice of local or cloud only comes up in the IDE and CLI. The question there is when to reach for the cloud.

Reach for a cloud session when the work fits remote execution better than your laptop:

- The task is long-running or autonomous, so an overnight refactor or a large migration does not depend on your machine staying awake.
...
- **Configurable network access:** you choose how much of the internet the sandbox can reach, and match the level to what the task actually needs.
    ...
    - Open internet: the agent has unrestricted internet access. Opening up network access raises risks like prompt injection and secret extraction inside the sandbox. The sandbox is still isolated in the cloud, so it cannot reach your local machine.
...
![A cloud session does real work safely: the agent runs in an isolated cloud sandbox with controlled access in and out. (1) Entering the session: nothing leaves your local machine; the sandbox clones your connected GitHub or GitLab repository. (2) During the session: the agent runs commands, edits code, installs dependencies, and works in your repositories inside an isolated sandbox, with configurable network access, scoped repository access, and encrypted secrets injected only at runtime. (3) Leaving the session: the agent opens a pull request and never pushes to main, so changes merge only after your review. Layers work together: the session is useful (the agent can do real work), controlled (you choose access and provide only needed secrets), and safer (isolation, network controls, and scoped access limit risk).](./cloud-session-safety.png)
...
> **Note:** Configuration Sync is managed from **Cloud configuration** in [Kiro Web settings](https://app.kiro.dev/settings/cloud-config). For the settings that shape the sandbox itself, including network access, environment variables, and setup commands, see the [Sandbox](https://kiro.dev/docs/web/sandbox/) documentation.
...
The easiest way to understand a cloud session is to watch one. This walkthrough starts a task from Kiro Web, shows what happens inside the cloud session, and follows the work through to a pull request, so you can see what runs remotely, what stays on your machine, and where you stay in control.
